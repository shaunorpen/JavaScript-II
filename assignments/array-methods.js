...
// Problem 1
// Produce an alphabetical array of all runners by surname

const sortedRunnersSurnames = runners.map(runner => runner.last_name).sort();
console.log(sortedRunnersSurnames);

// Problem 2
// Find the average donation per runner
const averageDonation = runners.reduce((acc, runner) => acc + runner.donation, 0) / runners.length
console.log(averageDonation);

// Problem 3
// Find the total number of each shirt size we need to order
const shirtSizes = runners.map(runner => runner.shirt_size);
console.log(shirtSizes);
const uniqueShirtSizes = shirtSizes.filter((item, index, array) => array.indexOf(item) === index);
console.log(uniqueShirtSizes);
const shirtOrder = uniqueShirtSizes.map(shirtSize => {
  countShirtSizes = (shirtSize) => shirtSizes.filter(shirt => shirt === shirtSize).length
  return [shirtSize, countShirtSizes(shirtSize)]
});
